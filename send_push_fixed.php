    ...
    foreach ($subscriptions as $sub) {
        try {
            ...
            if ($report->isSuccess()) {
                ...
                $logStmt = $pdo->prepare("
                    INSERT INTO notification_log (user_card, subscription_id, notification_type, payload, sent_at, status)
                    VALUES (?, ?, ?, ?, NOW(), 'sent')
                ");
                $logStmt->execute([
                    $user_card,
                    $sub['id'],
                    $data['tipo_notificacao'] ?? 'general',
                    $payload
                ]);
                ...
                // Log do erro
                $logStmt = $pdo->prepare("
                    INSERT INTO notification_log (user_card, subscription_id, notification_type, payload, sent_at, status, error_message)
                    VALUES (?, ?, ?, ?, NOW(), 'error', ?)
                ");
                $logStmt->execute([
                    $user_card,
