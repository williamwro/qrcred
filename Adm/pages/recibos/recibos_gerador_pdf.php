...
class PDF extends FPDF
{
    ...
    {
        // Logo
        $this->Image('logo.jpg',18,12,20);
        // Arial bold 15
        $this->SetFont('Arial','B',8);
        $this->Ln(3);
        $this->SetTextColor(0,0,156);
        $this->Cell(50);//move para direita 20 posiçoes
        $this->Write(0,utf8_decode('Caixa de Assistência à Saúde dos Servidores Públicos da Prefeitura Municipal de'));
        $this->Ln(4);
        $this->Cell(65);//move para direita 20 posiçoes
        $this->Write(0,utf8_decode('Varginha-MG, das Fundação, Autarquias e Camara Muncipal'));
        $this->Ln(4);
...
if ($categoria != 0 ) {
    PDF::setRS($categoria);
    $query = "Select nome_convenio as descri,sum(valor) as total,prolabore,divisao,
                     nomefantasia,cnpj,cpf,nome_categoria 
                From sind.qextrato 
               Where mes = '" . $mes_atual ."' 
                 and id_categoria_recibo = " . $categoria . " 
                 and divisao = ".$divisao." AND cobranca = true 
            Group by nome_convenio,divisao,prolabore,nomefantasia,cnpj,cpf,nome_categoria 
            order by nome_categoria ASC, nome_convenio ASC";
}else{
    PDF::setRS("TODOS");
    $query = "Select nome_convenio as descri,sum(valor) as total,prolabore,divisao,
                     nomefantasia,cnpj,cpf,nome_categoria 
...
$pdf->Output('I','RECIBOS'."-".$mes_atual."-QRCRED.pdf");