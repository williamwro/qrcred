...
try {
    ...
    // Buscar dados do associado - CORRIGIDO: Simplificada para evitar erro de parâmetro ambíguo
    logDebug("Buscando dados do associado", ['matricula' => $matricula]);
    
    // Capturar parâmetros obrigatórios
    ...
    logDebug("🔍 [SQL] Query do associado", [
        'sql' => $sql_associado,
        'params' => $params
    ]);
    ...
    if (!$associado) {
        logDebug("❌ [ERRO] Associado não encontrado", ['matricula' => $matricula, 'params' => $params]);
        echo json_encode([
            'success' => false,
